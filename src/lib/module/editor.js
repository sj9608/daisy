...
class ANEditor {
    ...
    addLabelObject({ xmin, ymin, xmax, ymax, class_name, translate = true, color = 'red', lineWidth = 2 }) {
        ...
        try {
            ...
            while (_index < _ext.length) {
                ...
                if (_bolb.size > 1024) {
                    ...
                    //이미지 오브잭트 만들기 
                    _imgElement = await new Promise((resolve, reject) => {
    ...
    async saveVocLabelData(uploadName) {
    ...
    async loadImageFromBolb(_blob, filename) {
    ...
    async saveImage({ baseAddr, uploadName, uploadPath }) {
